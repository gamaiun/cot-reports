    ...
    const fetchPriceData = async () => {
      try {
        const res = await fetch(
          "http://127.0.0.1:5000/api/data/agri?ticker=SUGAR%20NO.%2011%20-%20ICE%20FUTURES%20U.S.&column=SB=F_Close"
        );
  ...
  useEffect(() => {
    const fetchCotData = async () => {
      try {
        setLoadingCot(true);
        const res = await fetch(
          `http://127.0.0.1:5000/api/data/agri?ticker=SUGAR%20NO.%2011%20-%20ICE%20FUTURES%20U.S.&column=${encodeURIComponent(
            selectedColumn
          )}`
        );
        const data = await res.json();
