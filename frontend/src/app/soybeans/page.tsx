...
const SoybeansPage: React.FC = () => {
  ...
  // Fetch soybeans prices
  useEffect(() => {
    const fetchPriceData = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/data/agri?ticker=SOYBEAN%20CSO%20-%20CHICAGO%20BOARD%20OF%20TRADE&column=YF_Price`
        );
    ...
    const fetchCotData = async () => {
      try {
        ...
        const res = await fetch(
          ...
          }/api/data/agri?ticker=SOYBEAN%20CSO%20-%20CHICAGO%20BOARD%20OF%20TRADE&column=${encodeURIComponent(
            selectedColumn
          )}`
        );
        const data = await res.json();
    ...
    >
      ...
      >
        <div data-theme="dark" data-neutral="gray" data-brand="violet">
          {/* Top Chart: Soybeans Prices */}
          <div>
            <h2>Soybeans Prices</h2>
            {loadingPrice ? (
