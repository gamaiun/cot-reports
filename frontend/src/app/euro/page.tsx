    ...
    const fetchPriceData = async () => {
      try {
        ...
        const res = await fetch(
          "http://127.0.0.1:5000/api/data/currency_prices?ticker=EURO%20FX%20-%20CHICAGO%20MERCANTILE%20EXCHANGE"
        );
  ...
  useEffect(() => {
    const fetchCotData = async () => {
      try {
        setLoadingCot(true);
        const res = await fetch(
          `http://127.0.0.1:5000/api/data/currency_combined?ticker=EURO%20FX%20-%20CHICAGO%20MERCANTILE%20EXCHANGE&column=${encodeURIComponent(
            selectedColumn
          )}`
        );
        const data = await res.json();
