    ...
    const fetchPriceData = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/data/agri?ticker=COTTON%20NO.%202%20-%20ICE%20FUTURES%20U.S.&column=YF_Price`
        );
    ...
    const fetchCotData = async () => {
      try {
        ...
        const res = await fetch(
          ...
          }/api/data/agri?ticker=COTTON%20NO.%202%20-%20ICE%20FUTURES%20U.S.&column=${encodeURIComponent(
            selectedColumn
          )}`
        );
        const data = await res.json();
    ...
    >
      ...
      >
        <div data-theme="dark" data-neutral="gray" data-brand="violet">
          {/* Top Chart: Cotton Prices */}
          <div>
            <h2>Cotton Futures Prices</h2>
            {loadingPrice ? (
