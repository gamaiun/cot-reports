    ...
    const fetchPriceData = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/data/currency_prices?ticker=JAPANESE%20YEN%20-%20CHICAGO%20MERCANTILE%20EXCHANGE`
        );
    ...
    const fetchCotData = async () => {
      try {
        ...
        const res = await fetch(
          ...
          }/api/data/currency_combined?ticker=JAPANESE%20YEN%20-%20CHICAGO%20MERCANTILE%20EXCHANGE&column=${encodeURIComponent(
            selectedColumn
          )}`
        );
        const data = await res.json();
